...
-->
  ...
  <xsl:template name="header">
    <!-- Scheme -->
    <skos:ConceptScheme
      rdf:about="http://geonetwork-opensource.org/inspire-theme">
      <dc:title>INSPIRE themes</dc:title>
      <dc:description>INSPIRE themes thesaurus for GeoNetwork opensource.
      </dc:description>
      <dc:creator>
        <foaf:Organization>
  ...
  <xsl:template name="debug">
    ...
    <xsl:if test="$debug=true()">
      <xsl:message><xsl:value-of select="$cd"/>|<xsl:value-of select="$msg"/>
      </xsl:message>
    </xsl:if>
